...
from oapackage import splitDir, splitFile, splitTag
...
print(f"oapackage: {oapackage}: {oapackage.version()}")
...
def make_extend(lvls, splitdata, dosplit=False, verbose=1, makelog=True):
    ...
    basecmd = (
        "cd %s; " % outputdir
        + "mkdir -p %s;" % os.path.join(outputdir, splitdirx)
        + "oaconference -N %d -k %d -f AB --ctype 2 --itype 3  --j1zero 1 --j3zero 1 -i %s -o %s --select 3 "
    ...
    if verbose:
        print(f"make_extend {tag}: startfile {startfile}")
    if verbose >= 2:
        print("make_extend: cmd %s" % basecmd)
    cmd = basecmd
    # oapackage.runcommand(basecmd)
    if verbose:
        print(f"make_extend {tag}: checkfile {checkfile0}")
    if dosplit:
...
def split_file(lvls, splitdata, verbose=1):
    ...
    if level in splitdata:
        ...
        k = splitdata["kmax"]
    splitfile0 = os.path.join(splitDir(lvls), basetag + "-" + splitFile(lvls) + "-%d-%d" % (N, k) + ".oa")
    ...
    splitcmd = "cd %s; " % outputdir + "oasplit -n %d -f B -i %s -o %s" % (
        nsplit,
        splitfile,
        os.path.join(sdir, basetag + "-" + spl + "-sp%d" % level),
    )
    if verbose:
        print("split_file: split %s into %d files" % (splitfile0, nsplit))
    checkfile = os.path.join(outputdir, sdir, basetag + "-" + splitFile(lvls + [nsplit - 1]) + ".oa")
...
splitcmd = "cd %s; " % outputdir + "oasplit -n %d -f D -i %s -o %s" % (splitdata[0]["n"], checkfile, "dconference-sp0")
...
for ii in range(splitdata[0]["n"]):
    ...
    if oapackage.checkFilesOA(startfile) and not oapackage.checkFilesOA(checkfile):
        print(f"make job for extend of {lvls}")
    # create job file
    j = job(cmd, jobtype="extend %s" % tag, checkfiles=[checkfile], checkfilesstart=[startfile])
    alljobs += [j]

    splitcmd, checkfile, startfile, r = split_file([ii], splitdata, verbose=0)
    j = job(splitcmd, jobtype="split %s" % tag, checkfiles=[checkfile], checkfilesstart=[startfile])
    if j.canrun() and not j.complete():
        print(f"make job for split of {lvls}")
    alljobs += [j]

    cmd = researchOA.gzipOA(os.path.join(outputdir, splitDir(lvls)), unzip_text=True, cmdverbose=True)
    jc = job(cmd, jobtype="compress %s" % tag, checkfiles=[checkfile + ".gz"], checkfilesstart=[checkfile])
    jc.compressjob = True
...
for ii in range(splitdata[0]["n"]):
    ...
    for jj in range(splitdata[1]["n"]):
        ...
        jc = job(cmd, jobtype="compress %s" % tag, checkfiles=[checkfileZ + ".gz"], checkfilesstart=[checkfile])
        jc.compressjob = True
