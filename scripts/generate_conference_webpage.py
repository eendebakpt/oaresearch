...
from oaresearch.research_conference import (
    SingleConferenceParetoCombiner,
    calculateConferencePareto,
    cdesignTag,
    conference_design_has_extensions,
    conferenceDesignsPage,
    conferenceResultsFile,
    createConferenceDesignsPageParetoTable,
    generate_conference_latex_tables,
...
if 0:
    ...
    page = conferenceDesignsPage(
        pareto_results, verbose=1, makeheader=True, htmlsubdir=conference_html_dir, html_template=html_template
    )
    dt = time.time() - t0
...
if 0:
    from oaresearch.research_conference import calculateConferencePareto
...
def DconferencePage(
    ...
    if tdstyle is None:
        if ta == "left":
            ...
            tar = "left"
        tdstyle = "text-align:{}; margin-{}: 1em; padding-{}:1em; margin-{}: 1px;".format(ta, tar, tar, ta)
    ...
    page.th(["Number of rows"], colspan=len(Nrange) + 0, style=tdstyle)
    page.tr.close()
    page.tr()
...
if generate_webpage:
    ...
    ss += "For all the cases a set of representatives for the isomorphism classes is available. The algorithm used to generate the results is described in {} and {}. ".format(
    ...
    if 1:
        ...
        DconferencePage(
            ...
            tdstyle=tdstyle,
            tableclass="conftable conftable1",
            subpages=generated_subpages["cdesign"],
            conference_html_dir=conference_html_dir,
        )
    ...
    if 1:
        ...
        DconferencePage(
            ...
            kmax=27,
            Nstep=4,
            tableclass="conftable conftable2",
            conference_html_dir=conference_html_dir,
        )
        page.p()
        ...
        if 1:
            ...
            r = DconferencePage(
                ...
                tag="dconferencej1",
                kmax=14,
                tableclass="conftable conftable3",
                conference_html_dir=conference_html_dir,
            )
        ...
        if 1:
            ...
            DconferencePage(
                ...
                kmax=14,
                Nmax=24,
                tableclass="conftable conftable4",
                conference_html_dir=conference_html_dir,
            )
        ...
        if 1:
            ...
            DconferencePage(
                ...
                Nmax=20,
                kmax=13,
                tableclass="conftable conftable5",
                conference_html_dir=conference_html_dir,
            )
        ...
        if 1:
            ...
            page.p()
            page.add("For the isomorphism class level-permutatations of the rows are allowed.")
            page.add("We also allow multiple zeros in a row.")
            page.add(
    ...
    if 0:
        ...
        DconferencePage(
            page, tag="dconferencej1j3", Nstart=6, Nmax=81, kmax=28, Nstep=4, conference_html_dir=conference_html_dir
        )
        ...
        page.add("The square double conference matrices are weighing matrices of type W(N, N-2).")
        page.p.close()
...
def createConferenceDesignsPageParetoTableDSD(page, pareto_results, verbose=0, htmlsubdir=None):
    ...
    if pareto_results["narrays"] > 0 and pareto_results.get("full_results"):
        ...
        header = ["k", "Index design file", "r_2FIandQE", "r<sub>2FI</sub>", "F<sub>4</sub>", "B<sub>4</sub>"]
        if add_extra:
...
pdir = tempfile.mkdtemp(prefix="D_X_k")
...
page.init(
    title="Pareto optimal designs for D[*](N, k)",
    css=("oastyle.css"),
    lang="en",
    header="",
    htmlattrs=dict({"xmlns": "http://www.w3.org/1999/xhtml"}),
...
page.h1("Pareto optimal designs for D[*](N, k)", style="margin-bottom: 0.1em;")
page.p('<em style="color:darkblue;">pieter.eendebak@gmail.com</em>', style="margin-top: 0px;")
