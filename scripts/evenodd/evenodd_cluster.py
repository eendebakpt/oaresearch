...
from oaresearch.pythondevelop.researchOA import (analyseFile, checkFiles,
                                                 checkLevel, combineNumbers,
                                                 createJobScript, doSplitFile,
                                                 evenoddAnalysePartialRun,
                                                 evenoddAnalyseRun,
                                                 evenoddCases, findfiles,
                                                 gatherFilesList,
                                                 gatherResults, gzipOA, job,
                                                 jobStatus, loadAnalysisFile,
                                                 mkdirc, numbersFile,
                                                 parseParetoList, runcommand,
                                                 splitBase, splitdir,
                                                 splitFile, splitname,
                                                 splitTag)
...
parser.add_argument("--discardJ5", type=int, default=-1,
                    help="discard arrays with J5=max after specified number of columns")
...
print('--- Starting: case %d, dryrun %d, dryrun0 %d, compresssubdir %d' %
      (dobigcase, dryrun, dryrun0, compresssubdir))
...
outputdir = oapackage.mkdirc(os.path.join(
    resultsdir, 'eocluster-%s-%d-t%d' % (base, N, strength)))
...
if oapackage.checkFilesOA(os.path.join(outputdir, afile)):
    ...
    oapackage.writearrayfile(
        os.path.join(outputdir, afile), al, oapackage.ABINARY)

cmdlogfile = '%s-extend.txt' % splitBase([])
cmd = 'cd %s; oaextendsingle -f B -l 2 -m %d -c oaconfig-start.txt --maxk 5 | tee %s' % (
    outputdir, MODE_J5ORDERX, cmdlogfile)

j = job(cmd, jobtype='extend initial', checkfiles=[
        join(outputdir, afile5)], checkfilesstart=[join(outputdir, afile)])
...
j = job(cmd=cmd, checkfiles=afile,
        checkfilesstart=eostartfile, jobtype='extend initial')
j.execute = True
res = j.runjob(cache=cache)

os.chdir(outputdir)
dt0 = oahelper.parseProcessingTime(cmdlogfile)
print('time initial extension: %.1f [s], %d arrays ' % (
...
j = job(cmd=cmd, checkfiles=rfile, checkfilesstart=afile,
        jobtype='split level base')
j.execute = True
res = j.runjob(cache=cache)
...
for ii in iisel:
    ...
    if 1:
        j = job(cmd=cmd, jobtype='extend %s' % tag, shorttag='E%s' % tag, checkfiles=[
                os.path.join(outputdir, afile)], checkfilesstart=[rfile])
        # j.execute=True
        ...
        lvls = [ii]
        jjs, _, _ = generateLevelNext(adata, lvls, splitdata, outputdir, allcmdlogfile=cmdlogfile,
                                      discardJ5=discardJ5, paretomethod=paretomethod, ncores=4, priorextend=False, verbose=1)
        for jx in jjs:
            jx.queue = 'q24h'
...
if args.statistics:
    ...
    if nfiles > 0:
        ...
        print('stage 1: %d/%d files extended, # arrays %d (estimate %e)' %
              (nfiles, splitdata[0]['n'], ntotal, ntotal * sfac))
        print('  time %.1f [s], estimate %.1f [h] ' %
              (totaltime, float(totaltime) * sfac / 3600.))
        print('  time estimated: cluster %.1f [d]' % (
            float(totaltime) * sfac / (100 * 24. * 3600.)))
...
for ii in iisel:
    ...
    for jj in jjsel:
        ...
        joblist, runcomplete2, cmd2 = generateLevelNext(
            adata, lvls2, splitdata, outputdir, allcmdlogfile=cmdlogfile, splitmode=args.split, ncores=8, priorextend=False, discardJ5=discardJ5)
        alljobs += joblist
        ...
        if checkFiles(legacyfile):
            if verbose >= 2:
                print(
                    'generateLevelNext: %s: legacy format (generation complete)' % splitTag(lvls2))
            continue
        lockfile = join(outputdir, edir, 'lockfile-%s.txt' % tag2)
        if checkFiles(lockfile):
            if verbose >= 1:
                print('  !! generateLevelNext: %s: lockfile exists!' %
                      splitTag(lvls2))
            continue
        joblistg = gatherResults(
            lvls2, outputdir, splitdata, adata=adata, ncores=10, verbose=2)
        alljobs += joblistg
    ...
    print('  completed generation of block %d (runcomplete %d), collecting results' % (
        ii, runcomplete))
    ...
    joblistg1 = gatherResults(
        lvls1, outputdir, splitdata, adata=adata, verbose=2, ncores=4, dozip=args.dozipsub)
    alljobs += joblistg1
...
for kk in range(5, splitdata['kinitial'] + 1):
    ...
    if oapackage.checkFiles(outfile):
        continue
    parseParetoList(outfile, plist, verbose=0, afmode=oapackage.ATEXT,
                    nrows=N, ncols=kk, paretomethod=paretomethod, cache=cache)
...
joblistg1 = gatherResults(
    [], outputdir, splitdata, adata=adata, dozip=False, verbose=2)
#[j.runjob() for j in joblistg1]
...
if 0:
    ...
    if oahelper.checkFiles(os.path.join(outputdir, edir, mdfile), cache=cache) and 1:
        ...
        researchOA.arrayfilesMD5(
            os.path.join(outputdir, edir), os.path.join(outputdir, edir, mdfile), verbose=1)
