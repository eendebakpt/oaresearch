...
class job:
    ...
    def __repr__(self):
        ...
        if c:
            ...
            s = "job: %s: complete %d (canrun %d)" % (self.jobtype, c, self.canrun())
    ...
    def runjob(self, cache=True, verbose=None):
        ...
        if oahelper.checkFilesOA(self.checkfiles, cache=cache):
            ...
            if verbose >= 2:
                print(f"job {self.jobtype}: canrun() {self.canrun()}")
            if self.canrun():
                if verbose >= 2:
                    print("job %s: running command" % self.jobtype)

                res = oahelper.runcommand(self.cmd, dryrun=(not self.execute), idstr=self.jobtype, logfile=self.logfile)
                if not res == 0:
                    goodcalc = 0
...
def createJobScript(j, index=0, scriptdir=None, queue="q7d", verbose=1, jobtag="quickjob"):
    ...
    if vsccluster:
        if ncores == 8 or ncores == 6 or ncores == 12 or ncores == 14 or ncores == 18:
            print("warning: jobs on hopper should have number of cores compatible with 20")
    ...
    fid.write(f'echo "job: {j.jobtype}" >> {metafile} \n')
    fid.write('echo "host: ${HOSTNAME}, date: ${DATE}" >> %s \n' % metafile)
    fid.write('echo "   PBS_JOBID $PBS_JOBID, PBS_JOBNAME $PBS_JOBNAME, PBS_QUEUE $PBS_QUEUE" >> %s \n' % metafile)
    ...
    fid.write(f'echo "job {j.jobtype}: date: ${{DATE}}, done" >> {metafile} \n')
    ...
    if vsccluster and (queue is not None) and (queue != "none"):
        if queue == "q1h":
            ...
            wt = "168:00:00"
        substr = "qsub {} -q {} -l walltime={} {}".format(resstr, queue, wt, outfile)
    else:
