...
def Dvalue2col(A, k, kout, Lmax=1):
    ...
    dc = kout - k
    Aout = (A ** (float(m)) * (Lmax**dc)) ** (1.0 / mout)
...
def processBlocks(nblocks, outfiles, analysisfiles, verbose, cverbose, cache=1, cmdfile=None):
    ...
    for nn in range(0, nblocks):
        ...
        cmd = 'echo "block %d"; nice oaextendsingle -f B -r %s -o res%d -l 2 | tee log-base%d.txt' % (nn, afile, nn, nn)
...
def parseABarray(af, nn, verbose=1):
    ...
    for ii in range(0, nn):
        ...
        if A > 0:
            if verbose >= 2:
                print("array %d: D-efficiency %.3f A-efficiency %.3f" % (ii, A, B))
        a[ii] = A
        b[ii] = B
        asub[ii] = As
        bsub[ii] = Bs
